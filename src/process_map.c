...
/*   Updated: 2025/07/07 19:44:04 by tborges-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	count_map_lines(char *file)
{
    ...
    while (line != NULL)
    {
        // Se a linha começa com '1' ou é só de '1' e espaços, é uma linha de mapa
        if (line[0] == '1' || (line[0] == ' ' && ft_strchr(line, '1')))
            map_started = true;

        // Só conta linhas depois que o mapa realmente começou
...
static void	read_map_lines(t_map_errors *map_errors, char *file, int map_lines)
{
    ...
    while (line != NULL)
    {
        // Detecta o início do mapa real
        if (line[0] == '1' || (line[0] == ' ' && ft_strchr(line, '1')))
            map_started = true;

        // Só pega linhas depois que o mapa realmente começou
        if (map_started && (line[0] == '1' || line[0] == ' ' ||
                           ft_strchr("NSEW", line[0])))
        {
